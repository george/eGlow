...
public class DataManager implements PluginMessageListener {
	...
	public static void initialize() {
		addEGlowEffects();
		EGlow.getEGlowInstance().getServer().getMessenger().registerOutgoingPluginChannel(EGlow.getEGlowInstance(), "eglow:bungee");
		EGlow.getEGlowInstance().getServer().getMessenger().registerIncomingPluginChannel(EGlow.getEGlowInstance(), "eglow:bungee", new DataManager());
	}
	...
	public static void addCustomEffects() {
		...
		for (String effectName : Effect.GET_ALL_EFFECTS.get()) {
			...
			if (!oldEffects.isEmpty() && oldEffects.contains(effectName.toLowerCase())) {
				...
				try {
					EGlow.getEGlowInstance().getServer().getPluginManager().addPermission(new Permission(permission, "Activate " + effectName + " effect.", PermissionDefault.FALSE));
	...
	public static void TABProxyUpdateRequest(Player player, String glowColor) {
		...
		Bukkit.getServer().sendPluginMessage(EGlow.getEGlowInstance(), "eglow:bungee", out.toByteArray());
	}

	public static void sendAPIEvent(EGlowPlayer player, boolean fake) {
		if (fake) return;
		new BukkitRunnable() {
			@Override
			public void run() {
