				...
				public void write(ChannelHandlerContext context, Object packet, ChannelPromise channelPromise) throws Exception {
					if (NMSHook.nms.PacketPlayOutScoreboardTeam.isInstance(packet)) {
						if (DebugUtil.TABInstalled()) {
							if (EGlow.getEGlowInstance().getTABAddon() == null) {
								super.write(context, packet, channelPromise);
								return;
							} else {
								if (!EGlow.getEGlowInstance().getTABAddon().isVersionSupported() || EGlow.getEGlowInstance().getTABAddon().blockEGlowPackets() || DebugUtil.isTABBridgeInstalled()) {
									super.write(context, packet, channelPromise);
									return;
								}
							}
					...
					if (NMSHook.nms.PacketPlayOutEntityMetadata.isInstance(packet)) {
						...
						if (glowingEntities.containsKey(entityID)) {
							...
							//TODO try to modify send metadata instead of getting original metadata
