...
public class EGlowMainConfig {
	...
	public static void initialize() {
		configFile = new File(EGlow.getEGlowInstance().getDataFolder(), "Config.yml");
		...
		try {
			...
			if (!configFile.exists()) {
				...
				EGlow.getEGlowInstance().saveResource("Config.yml", false);
			} else {
			...
			if (!config.isConfigurationSection("Command-alias")) {
				File oldFile = new File(EGlow.getEGlowInstance().getDataFolder(), "OLDConfig.yml");
	...
	private static void registerCustomPermissions() {
		...
		for (String name : MainConfig.SETTINGS_JOIN_FORCE_GLOWS_LIST.getConfigSection()) {
			try {
				EGlow.getEGlowInstance().getServer().getPluginManager().addPermission(new Permission("eglow.force." + name.toLowerCase()));
